...
import pandas as pd
import codecs
this_root = '/Volumes/SSD/wenq/给小李0716/'
...
def gen_region_point_dic():
    shp = '/Volumes/SSD/wenq/给小李0716/湖北边界/hb_polygon.shp'
    csv = '/Volumes/SSD/wenq/给小李0716/cmip6数据/GFDL-CM4_245.csv'
    outf = '/Volumes/SSD/wenq/给小李0716/result/hubei_point.txt'
    df = pd.read_csv(csv)
    sta = {}
    ...
    for key in tqdm(sta):
        x,y = key
        # print(x,y)
        in_shp = check_points_in_shp(shp,x,y)
        # print(in_shp)
        if not in_shp:
            key_in = (x,y)
            # print(key_in)
            in_shp_sta.append(key_in)
    fw = open(outf,'w')
    fw.write(str(in_shp_sta))
    fw.close()


def get_hb_point():
    f = '/Volumes/SSD/wenq/给小李0716/result/hubei_point.txt'
    p_list = eval(open(f,'r').read())
    # for x,y in p_list:
    #     plt.scatter(x,y)
    # plt.show()
    return p_list
...
class Every_Model:
    ...
    def read_historical(self):
        region_pixs = get_hb_point()
        outdir = this_root + 'result/historical/各模式逐年区域平均序列/HB/'
    ...
    def read_SSP245(self):
        region_pixs = get_hb_point()
        outdir = this_root + 'result/SSP245/各模式逐年区域平均序列/HB/'
    ...
    def read_SSP585(self):
        ...
        #
        region_pixs = get_hb_point()
        outdir = this_root + 'result/SSP585/各模式逐年区域平均序列/HB/'
...
class Every_Model_time:
    ...
    def historical(self):
        ...
        region_pixs = get_hb_point()
        outdir = this_root + 'result/historical/年代值/HB/各模式/'
...
class Every_Model_time_mean:
    ...
    def historical(self):
        ...
        fdir = this_root + 'result/historical/年代值/HB/各模式/'
        outdir = this_root + 'result/historical/年代值/HB/各模式平均/'
